-- Generic WhatsApp inbound signup support for tenants with monthly billing.
-- This workflow is intentionally not tied to a specific business type.
...
grant execute on function public.wa_billing_signup_load_or_create_context(uuid, text, text, text, jsonb) to authenticated, service_role;
grant execute on function public.wa_billing_signup_create_customer(uuid, text, text, integer, integer, uuid, text) to authenticated, service_role;
grant execute on function public.wa_billing_signup_conversation_patch(uuid, text, jsonb, boolean) to authenticated, service_role;
