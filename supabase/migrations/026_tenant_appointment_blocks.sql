-- Tenant-wide date/time blocks for holidays, absences and temporary closures.

create table if not exists public.tenant_appointment_blocks (
  ...
  constraint tenant_appointment_blocks_interval_check check (ends_at > starts_at),
  constraint tenant_appointment_blocks_reason_check check (reason is null or char_length(reason) <= 240)
);

create index if not exists tenant_appointment_blocks_tenant_period_idx
on public.tenant_appointment_blocks(tenant_id, starts_at, ends_at);

alter table public.tenant_appointment_blocks enable row level security;

drop policy if exists tenant_appointment_blocks_service_role_all on public.tenant_appointment_blocks;
create policy tenant_appointment_blocks_service_role_all
on public.tenant_appointment_blocks for all
using (auth.role() = 'service_role')
with check (auth.role() = 'service_role');

drop policy if exists tenant_appointment_blocks_manage_own_tenant on public.tenant_appointment_blocks;
create policy tenant_appointment_blocks_manage_own_tenant
on public.tenant_appointment_blocks for all
using (exists (
  select 1 from public.tenant_users tu
  where tu.tenant_id = tenant_appointment_blocks.tenant_id
...
grant select, insert, update, delete on public.tenant_appointment_blocks to authenticated, service_role;

create or replace function public.prevent_appointment_during_tenant_block()
returns trigger
language plpgsql
...
begin
  ...
  if v_should_check and new.deleted_at is null
     and new.status not in ('cancelled', 'no_show')
     and exists (
       select 1 from public.tenant_appointment_blocks b
...
drop trigger if exists appointments_prevent_tenant_block on public.appointments;
create trigger appointments_prevent_tenant_block
before insert or update of tenant_id, starts_at, ends_at, status, deleted_at
on public.appointments
for each row execute function public.prevent_appointment_during_tenant_block();
...
as $$
  ...
  staff_candidates as (
    ...
    join public.tenant_service_staff_members link
      on link.staff_member_id = sm.id and link.tenant_id = sm.tenant_id and link.service_id = p_service_id
    where sm.tenant_id = p_tenant_id and sm.is_active = true
  ...
  available_slots as (
    ...
    and not exists (
      ...
      where b.tenant_id = p_tenant_id
        and tstzrange(b.starts_at, b.ends_at, '[)') && tstzrange(candidate_slots.starts_at, candidate_slots.ends_at, '[)')
    )
  ),
  ranked_slots as (
    select available_slots.*,
      row_number() over (partition by available_slots.starts_at order by available_slots.staff_member_name nulls last) as staff_rank
    from available_slots
  ),
  compact_slots as (
    select * from ranked_slots where staff_rank = 1
    order by starts_at asc, staff_member_name nulls last
    limit greatest(1, least(coalesce(p_limit, 5), 8))
    offset greatest(0, coalesce(p_offset, 0))
  )
  select row_number() over (order by compact_slots.starts_at asc)::integer,
    compact_slots.starts_at, compact_slots.ends_at,
    concat(
      to_char(compact_slots.starts_at at time zone p_timezone, 'Dy DD/MM'),
      ' as ', to_char(compact_slots.starts_at at time zone p_timezone, 'HH24:MI'),
      case when compact_slots.staff_member_name is null then '' else concat(' com ', compact_slots.staff_member_name) end
    ),
    compact_slots.staff_member_id, compact_slots.staff_member_name
  from compact_slots;
$$;
