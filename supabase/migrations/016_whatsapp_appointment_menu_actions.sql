...
drop function if exists public.wa_appointment_load_or_create_context(uuid, text, text, jsonb);
    ...
    coalesce((
      select jsonb_agg(
        jsonb_build_object(
          ...
          'label', concat(
            to_char(upcoming.starts_at at time zone 'America/Fortaleza', 'DD/MM'),
            ' as ',
            to_char(upcoming.starts_at at time zone 'America/Fortaleza', 'HH24:MI'),
            case
              when coalesce(upcoming.service_name_snapshot, upcoming.service_name) is null then ''
              else concat(' - ', coalesce(upcoming.service_name_snapshot, upcoming.service_name))
            end,
            case
              when coalesce(upcoming.staff_member_name_snapshot, upcoming.staff_member_name) is null then ''
              else concat(' com ', coalesce(upcoming.staff_member_name_snapshot, upcoming.staff_member_name))
            end
          ),
          'service_id', upcoming.service_id,
          'service_name', coalesce(upcoming.service_name_snapshot, upcoming.service_name),
          'staff_member_id', upcoming.staff_member_id,
          'staff_member_name', coalesce(upcoming.staff_member_name_snapshot, upcoming.staff_member_name),
          'duration_minutes', upcoming.duration_minutes,
          'customer_birth_date', upcoming.customer_birth_date
        )
      ...
      from (
        select
          ...
          a.staff_member_id,
          a.staff_member_name_snapshot,
          s.name as service_name,
          s.duration_minutes,
          sm.name as staff_member_name,
          coalesce(tc.birth_date, ec.birth_date) as customer_birth_date
        ...
        where a.tenant_id = t.id
          ...
          and a.starts_at >= now()
          and regexp_replace(coalesce(tc.phone_e164, ec.whatsapp_e164, ''), '\D', '', 'g') = regexp_replace(v_chat_id, '\D', '', 'g')
